...
module.exports = function(app, config={}){
  ...
  let opts = _.merge(defaults, config);
  ...
  app.use(function(req, res, next){
    let accountName = req.get('X-Authub-Account')
                || req.query.authub_account || "general"; //General account is the guest account
    logger.debug("account is : " + accountName);
    logger.debug("headers: ", req.headers);

    //Just keep accountName through the request lifecycle
    res.locals.account = accountName;

    /* general is a special Account that does not have a valid company account authorized to
      If the account is not "general", then we need to verify if it does exist.
    */
    if(accountName !== 'general'){
      dbs.connectToMaster(function(err, db){
        db.model("Account")
          .findOne({ name: accountName })
          .lean()
          .then(function(account){
              if(!account) throw new Error(accountName + " is not a valid account");
              return next();
          })
          .catch(function(err){
              logger.error(err);
              throw new Error("There are some problems to get account info, detail: " + err.message);
          });
      });
    }else{
      return next();
