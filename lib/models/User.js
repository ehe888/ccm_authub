...
UserSchema.statics.resetPasswordWhenForgot = function(username, newPassword) {
    let searchCondition = { username: username };
    if(validator.isEmail(username)){
      searchCondition = { email: username };
    }
    return this.findOne(searchCondition)
			.then( user => {
				if(user === null){
					return Promise.reject(new Error(reasons.INVALID_CREDENTIAL));
				}

				if (!user.activated){
					return Promise.reject(new Error(reasons.NOT_ACTIVATED));
				}

				if (user.isLocked) {
						return Promise.reject(new Error(reasons.MAX_ATTEMPTS));
				}
